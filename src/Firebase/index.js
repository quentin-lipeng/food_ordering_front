...
export const appFetchCategory = async () => {
  const allCategories = await fetch("/api/food/categories").then((resp) =>
    resp.json()
  );
  return allCategories.data;
};

// Fetch All Food Products  from back end
export const appFetchFoodsData = async (category) => {
  const allFoodsResp = await fetch("/api/food/category/" + category);
  const foodsJson = await allFoodsResp.json();
  return foodsJson.data;
};
